...
def splitTrainTest(data, trainProp):
    ...
    stopIndex = int(trainProp*len(data))
    #print(stopIndex)
    a = data[0:stopIndex]
    b = data[stopIndex:len(data)]
    '''for i in range(int(len(data)*trainProp)):
        a.append(data[i])

    for i in reversed(range(int(len(data)*(1-trainProp)))):
        
        b.append(data[len(data)-1-i])
    '''
    return (a,b)
...
def formatForClassifier(dataList, label):
    ...
    result = []
    for doc in dataList:
        result.append([format_sentence(doc),label])
    #print(result)
